...
static int Window_getTitle(lua_State* state);
static int Window_getWidth(lua_State* state);
static int Window_getHeight(lua_State* state);
static int Window_getRealTitle(lua_State* state);
static int Window_getRealWidth(lua_State* state);
static int Window_getRealHeight(lua_State* state);
...
static int RenderResources_createFont(lua_State* state);
static int RenderResources_deleteFont(lua_State* state);
static int RenderResources_getFont(lua_State* state);
...
void LuaHandler::bindWindow(Window* window)
{
	...
	lua_pushcfunction(m_State, Window_getTitle);
	lua_setfield(m_State, -2, "getTitle");
	lua_pushcfunction(m_State, Window_getWidth);
	lua_setfield(m_State, -2, "getWidth");
	lua_pushcfunction(m_State, Window_getHeight);
	lua_setfield(m_State, -2, "getHeight");
	lua_pushcfunction(m_State, Window_getRealTitle);
	lua_setfield(m_State, -2, "getRealTitle");
	lua_pushcfunction(m_State, Window_getRealWidth);
	lua_setfield(m_State, -2, "getRealWidth");
	lua_pushcfunction(m_State, Window_getRealHeight);
	lua_setfield(m_State, -2, "getRealHeight");
...
void LuaHandler::bindRenderResources(RenderResources* rres)
{
	...
	lua_pushcfunction(m_State, RenderResources_createFont);
	lua_setfield(m_State, -2, "createFont");
	lua_pushcfunction(m_State, RenderResources_deleteFont);
	lua_setfield(m_State, -2, "deleteFont");
	lua_pushcfunction(m_State, RenderResources_getFont);
	lua_setfield(m_State, -2, "getFont");
...
int Window_getTitle(lua_State* state)
{
	ASSERT(s_Window != NULL, "s_Window is nullptr!", 0);
	lua_pushstring(state, s_Window->getTitle());
	return 1;
}

int Window_getWidth(lua_State* state)
{
	ASSERT(s_Window != NULL, "s_Window is nullptr!", 0);
	lua_pushinteger(state, s_Window->getWidth());
	return 1;
}

int Window_getHeight(lua_State* state)
{
	ASSERT(s_Window != NULL, "s_Window is nullptr!", 0);
	lua_pushinteger(state, s_Window->getHeight());
	return 1;
}

int Window_getRealTitle(lua_State* state)
{
	ASSERT(s_Window != NULL, "s_Window is nullptr!", 0);
	lua_pushstring(state, s_Window->getTitle(true));
	return 1;
}

int Window_getRealWidth(lua_State* state)
{
	ASSERT(s_Window != NULL, "s_Window is nullptr!", 0);
	lua_pushinteger(state, s_Window->getWidth(true));
	return 1;
}

int Window_getRealHeight(lua_State* state)
{
	ASSERT(s_Window != NULL, "s_Window is nullptr!", 0);
	lua_pushinteger(state, s_Window->getHeight(true));
	return 1;
}
...
int RenderResources_createFont(lua_State* state)
{
	ASSERT(s_RenderResources != NULL, "s_RenderResources is nullptr!", 0);
	const char* path = luaL_checkstring(state, 1);
	std::string spath = s_DefPath + path;
	xg::Guid& guid = s_RenderResources->createFont(spath.c_str());
	lua_pushlightuserdata(state, &guid);
	return 1;
}

int RenderResources_deleteFont(lua_State* state)
{
	ASSERT(s_RenderResources != NULL, "s_RenderResources is nullptr!", 0);
	if (!lua_islightuserdata(state, 1))
		return 0;
	void* guid = lua_touserdata(state, 1);
	s_RenderResources->deleteFont(*(xg::Guid*)guid);
	return 0;
}

int RenderResources_getFont(lua_State* state)
{
	ASSERT(s_RenderResources != NULL, "s_RenderResources is nullptr!", 0);
	ASSERT(lua_islightuserdata(state, 1), "RenderResources_getShader did not recieve lightuserdata!", 0);
	void* guid = lua_touserdata(state, 1);
	RenderFontResource* res = s_RenderResources->getFont(*(xg::Guid*)guid);
	lua_pushlightuserdata(state, &res);
	return 0;
}
