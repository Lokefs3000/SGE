...
Renderer::Renderer(Window* window, RenderResources* res, bool forceDebug)
{
	...
	m_OpenGL = SDL_GL_CreateContext(window->getWindow());
	SDL_GL_MakeCurrent(window->getWindow(), m_OpenGL);
	gladLoadGL(SDL_GL_GetProcAddress);
	
	if (forceDebug) {
		DebugLogger::EnableGLLogging();
	}
...
void Renderer::draw()
{
	...
	if (m_LastWidth != m_Window->getWidth() || m_LastHeight != m_Window->getHeight()) {
		float w2 = m_Window->getWidth() / 2.0f;
		float h2 = m_Window->getHeight() / 2.0f;

		m_2DProjection = glm::ortho(-w2, w2, h2, -h2, -1.0f, 1.0f);

		m_Shaders.RectShader->Bind();
		m_Shaders.RectShader->SetMat4("uProjection", m_2DProjection);
	}

	m_LastWidth = m_Window->getWidth();
	m_LastHeight = m_Window->getHeight();
}
